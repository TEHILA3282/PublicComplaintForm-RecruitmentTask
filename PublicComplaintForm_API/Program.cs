...
builder.Services.AddSingleton<MonthlyInquiryReportService>();
...
app.MapGet(
    "/reports/monthly-inquiries",
    (
        DateTime? reportMonth,
        [FromServices] MonthlyInquiryReportService reportService,
        [FromServices] ILog log
    ) =>
    {
        var selectedMonth = reportMonth ?? DateTime.Today;

        var normalizedMonth = new DateTime(
            selectedMonth.Year,
            selectedMonth.Month,
            1
        );
        ...
        log.Info(
            $"Monthly inquiry report requested for {normalizedMonth:yyyy-MM}."
        );

        var report = reportService.GetMonthlyReport(normalizedMonth);

        return Results.Ok(new
        {
            reportMonth = normalizedMonth.ToString("yyyy-MM"),
